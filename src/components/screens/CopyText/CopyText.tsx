...
const CopyText: React.FC<CopyTextProps> = ({ states, sectionName }) => {
  ...
  const generateRtfContent = () => {
    ...
    const initialText = '{\\rtf1\\ansi\\ansicpg1252\\deff0\\nouicompat\\deflang1033{\\fonttbl{\\f0\\fnil\\fcharset0 Calibri;}}\n';
      ...
      <div className="copy-box">
        ...
        <div className="copy-text">
          ...
          <ul id="yourHtmlId" className="uper-list">
            {getQuestions && getQuestions?.map((item: any, i: number) => {

              // console.log("item?.followUpSelected", item?.questionType === "Free Text" );

              // type date and time 
              let dataTime = item?.selectedDate ? `Since ${item.selectedDate}` : null;
              let hours = item?.hours ? `last ${item?.hours + " " + item?.selectedOption}` : null;
              let dataTimeShow = item?.selectedOption ? hours : item?.selectedDate ? dataTime : null;
              // type date and time 
              let InputDosage = `${item?.dosageInput}mg ${item?.selectedRadioValue}  ${item?.selectedRadioValue2 ? item?.selectedRadioValue2 : ""}`
              let freeText = item?.freeText ? item?.freeText : item?.followUpSelected?.toString()
              ...
              if (dataTimeShow) {
                ...
                console.log("item?.followUpSelected", item?.followUpSelected[0]?.includes('-'));

                formattedItem = item?.followUpSelected[0]?.includes('-') ? item?.followUpSelected[0]?.split("-")[1] : item?.followUpSelected?.toString()
              }
              ...
              return (
                <div className="uper-li" key={i}>
                  {formattedItem ? formattedItem : null} <span style={{ display: 'none' }}>_</span>
                </div>
              );
            })}
          </ul>
